...
class AccountChangeFormTest(TestCase):
    ...
    def test_username_validity(self):
        ...
        form = AccountChangeForm(data, instance=user)
        self.assertFalse(form.is_valid())
        self.assertEqual(form['email'].errors,
                         [_('Enter a valid email address.')])
    ...
    def test_bug_14242(self):
        ...
        class MyUserForm(AccountChangeForm):
            def __init__(self, *args, **kwargs):
                super(MyUserForm, self).__init__(*args, **kwargs)
                self.fields['groups'].help_text = 'These groups give users different permissions'

            class Meta(AccountChangeForm.Meta):
                fields = ('groups',)
    ...
    def test_bug_19133(self):
        ...
        user = get_user_model().objects.get(email='testclient@example.com')
        form_for_data = AccountChangeForm(instance=user)
        ...
        form = AccountChangeForm(instance=user, data=post_data)
...
class AccountAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        if settings.AUTH_USER_MODEL == "custom_user.Account":
        ...
        elif settings.AUTH_USER_MODEL == "test_custom_user_subclass.MyCustomAccount":
            self.app_name = "test_custom_user_subclass"
            self.model_name = "mycustomAccount"
            self.model_verbose_name = "MyCustomAccountVerboseName"
            self.model_verbose_name_plural = "MyCustomAccountVerboseNamePlural"
            if django.VERSION[:2] < (1, 7):
